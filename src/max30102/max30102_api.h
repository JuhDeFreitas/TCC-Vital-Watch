...
// ---- Struct de configuração com bitfields ----
typedef struct {

    union {
        uint8_t data1;  // REG_INTR_ENABLE_1 (0x02)
        struct {
            uint8_t RESERVED      :4;
    ...
    union {
        uint8_t data2;  // REG_INTR_ENABLE_2 (0x03)
        struct {
            uint8_t RESERVED1         :1;
    ...
    union {
        uint8_t data3;  // REG_FIFO_WR_PTR (0x04)
        struct {
    ...
    union {
        uint8_t data4;  // REG_OVF_COUNTER (0x05)
        struct {
    ...
    union {
        uint8_t data5;  // REG_FIFO_RD_PTR (0x06)
        struct {
    ...
    union {
        uint8_t data6;  // REG_FIFO_CONFIG (0x08)
        struct {
    ...
    union {
        uint8_t data7;  // REG_MODE_CONFIG (0x09)
        struct {
            uint8_t MODE     :3;
            uint8_t RESERVED :3;
            uint8_t RESET    :1;
    ...
    union {
        uint8_t data8;  // REG_SPO2_CONFIG (0x0A)
        struct {
            uint8_t LED_PW       :2;
            uint8_t SPO2_SR      :3;
    ...
    union {
        uint8_t data9;  // REG_LED1_PA (0x0C)
        struct {
            uint8_t LED1_PA :8;
        } LED1_PULSE_AMP;
    };
    ...
    union {
        uint8_t data10; // REG_LED2_PA (0x0D)
        struct {
            uint8_t LED2_PA :8;
        } LED2_PULSE_AMP;
    };
    ...
    union {
        uint8_t data11; // REG_PILOT_PA (0x10)
        struct {
            uint8_t PILOT_PA :8;
        } PROX_LED_PULS_AMP;
    };
    ...
    union {
        uint8_t data12; // REG_MULTI_LED_CTRL1 (0x11)
        struct {
            uint8_t SLOT1     :3;
            uint8_t RESERVED2 :1;
            uint8_t SLOT2     :3;
    ...
    union {
        uint8_t data13; // REG_MULTI_LED_CTRL2 (0x12)
        struct {
            uint8_t SLOT3     :3;
            uint8_t RESERVED2 :1;
            uint8_t SLOT4     :3;
